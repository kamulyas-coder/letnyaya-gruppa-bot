...
Использует plain text (без Markdown) чтобы избежать ошибок парсинга
с именами участников, апострофами и спецсимволами.
"""
...
def build_weekly_digest():
    """Собирает полный еженедельный дайджест (plain text, без Markdown)."""
    lines = []
    lines.append("━━━━━━━━━━━━━━━━━━━━━━")
    ...
    try:
        ...
        if participants:
            ...
            lines.append("Данные из таблицы пока недоступны")
    except Exception as e:
        lines.append(f"Не удалось загрузить таблицу: {e}")

    lines.append("")

    # --- Блок 2: Streak-таблица ---
    lines.append("🔥 Streak — активных дней всего\n")
    ...
    if streak_table:
        ...
        lines.append("Пока нет данных об активности")
    ...
    lines.append("")
    lines.append("━━━━━━━━━━━━━━━━━━━━━━")
    lines.append("Следующий дайджест — в воскресенье 🙌")

    return "\n".join(lines)


def build_streak_only():
    """Только streak-таблица — для команды /streak (plain text)."""
    lines = []
    ...
    lines.append("")
    lines.append("Streak = общее число дней, когда ты писал в чат.")
    lines.append("Не сгорает при пропуске — растёт с каждым активным днём 💪")
    return "\n".join(lines)
