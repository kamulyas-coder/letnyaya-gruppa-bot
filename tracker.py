...
def record_message(user_id, name, username, text):
    ...
    if today not in active_days:
        ...
        if streak % 7 == 0 and streak > 0:
            member["points"] += 5  # бонус за неделю без пропуска
    ...
    entry = {
        ...
        "user_id": str(user_id),
        "name": name,
        "text": text[:300],  # обрезаем очень длинные
    }
...
def _calc_streak(active_days_sorted):
    """Считает текущий streak подряд идущих дней."""
    if not active_days_sorted:
        return 0
    today = date.today()
    streak = 0
    current = today
    days_set = set(active_days_sorted)
    while current.isoformat() in days_set:
        streak += 1
...
def get_streak_table():
    ...
    for uid, m in data["members"].items():
        streak = _calc_streak(m.get("active_days", []))
