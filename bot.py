...
BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "8961294024:AAHjP1BG4vhpV_aGovs3uguBf-7wabqid-4")
CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "")  # заполняется после первого сообщения в чат
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        f"Я слежу за активностью в чате, считаю streak'и и каждое воскресенье "
        f"отправляю дайджест с прогрессом группы.\n\n"
        f"📌 *Chat ID этой группы:* `{chat.id}`\n"
        f"_(сохрани это в .env как TELEGRAM\\_CHAT\\_ID)_\n\n"
        f"Доступные команды:\n"
        f"/digest — дайджест прямо сейчас\n"
        f"/streak — таблица streak'ов\n"
...
async def cmd_digest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⏳ Собираю данные...", parse_mode="Markdown")
    try:
...
async def cmd_mystats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from digest import streak_emoji
    emoji = streak_emoji(stats["streak"])
    text = (
        f"*Твоя статистика, {stats['name']}* 📈\n\n"
        f"🔥 Streak: *{stats['streak']} дн.* {emoji}\n"
        f"🏆 Рекорд: *{stats['longest_streak']} дн.*\n"
        f"⭐ Очки: *{stats['points']}*\n"
        f"💬 Сообщений: *{stats['messages_count']}*\n\n"
        f"_Пиши в чат каждый день — и streak горит!_"
    )
    await update.message.reply_text(text, parse_mode="Markdown")
...
async def cmd_goals(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⏳ Читаю таблицу...", parse_mode="Markdown")
    try:
        ...
        for p in participants:
            ...
            for g in p["goals"]:
                ...
                if g.get("deadline"):
                    lines.append(f"     _дедлайн: {g['deadline']}_")
            lines.append("")
...
async def track_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Реакция на streak-достижения (milestone'ы)
    stats = get_member_stats(user.id)
    if stats:
        streak = stats["streak"]
        if streak in (3, 7, 14, 21, 30):
            ...
            if congrats:
                await update.message.reply_text(congrats, parse_mode="Markdown")


def _streak_congrats(name, streak):
    messages = {
        3:  f"🔥 *{name}*, 3 дня подряд! Хорошее начало!",
        7:  f"🔥🔥 *{name}*, неделя без пропусков! Огонь! +5 очков бонусом",
        14: f"🔥🔥🔥 *{name}*, две недели подряд! Это уже привычка! +5 очков",
        21: f"⚡ *{name}*, 21 день! Говорят, именно столько нужно для привычки 🧠",
        30: f"🏆 *{name}*, МЕСЯЦ БЕЗ ПРОПУСКОВ! Легенда! +5 очков",
    }
